...
from .shared import PipelineComponent, strip_credentials
...
def origin_repo_url() -> str | None:
    ...
    # CI rewrites origin to embed a token; this reference ships inside the wheel.
...
def lookup_pypi_license(name: str) -> LicenseRepository | None:
    ...
    """
    try:
        # metadata() returns an email.message.Message at runtime; typeshed only
        # exposes `.get` on the PackageMetadata protocol from 3.12, so cast to the
        # concrete type to keep the type checker happy on the 3.10/3.11 floor.
        meta = cast(Message, metadata(name))
    except PackageNotFoundError:
        return None
...
def validate_sbom(target: Path = SBOM_FILE) -> list[str]:
    ...
    Returns a list of error messages, empty when the SBOM is valid.

    This used to run in a `uv run python` subprocess. The launcher puts the vendored CI
    libraries earlier on `sys.path` than the venv, and the vendored tree carried its own
    `jsonschema` — so an in-process import resolved to that copy instead of the one
    cyclonedx-python-lib validates against. `jsonschema` was never wanted there; it arrived
    as a transitive dependency of the vendoring toolchain, which is no longer vendored. With
    it gone the import resolves to the venv and the subprocess has nothing left to isolate.
    """
    validator = JsonStrictValidator(SchemaVersion.V1_7)
    error = validator.validate_str(target.read_text(encoding='utf-8'))
