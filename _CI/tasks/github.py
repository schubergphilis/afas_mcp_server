...
from .shared import PipelineComponent, container_engine, execute, image_digest_reference, strip_credentials
...
def publish_deps_image(context: Context, tag: str) -> str:
    """Build and push the deps image to ghcr.io. Return a digest-pinned reference.

    Logs into the registry with the detected engine and builds+pushes only when the
    image is not already present. The tag is content-addressed on the lockfile, the
    Dockerfile and the base image, so a hit means those inputs match.

    Reusing a pre-existing tag is a deliberate trade-off: it keeps CI fast, but it
    trusts whoever pushed that tag first. Restrict registry write access to this
    pipeline accordingly — see the deps-image section of the CI framework reference.
    The returned reference is digest-pinned either way, so the tag cannot be
    repointed between this job and the jobs that run inside the image.
    """
    ...
    if result and not result.failed:
        ...
        # Pull so a repo digest exists locally to pin against, and so a corrupt or
        # unpullable image fails here rather than in every consumer job.
        execute(context, f'{engine} pull {image}')
    else:
    ...
    return image_digest_reference(context, engine, image)
...
def origin_slug(context: Context) -> str:
    ...
    if remote is None or remote.failed:
        return ''
    # In CI origin carries a token (https://x-access-token:<token>@github.com/…),
    # which the patterns below would otherwise fail to match.
    url = strip_credentials(remote.stdout.strip())
    match = re.match(
...
def create_release_pr(context: Context, release_branch: str, new_version: str) -> str:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            'User-Agent': 'afas_mcp_server-release',
        },
    )
    try:
...
def request_pages_build(context: Context) -> None:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            'Authorization': f'Bearer {token}',
            'X-GitHub-Api-Version': '2022-11-28',
            'User-Agent': 'afas_mcp_server-docs',
        },
    )
    try:
        with urllib.request.urlopen(request, timeout=30) as response:  # noqa: S310
            print(f'Requested GitHub Pages build (HTTP {response.status}).')
    except urllib.error.HTTPError as exc:
        detail = exc.read().decode('utf-8', errors='replace')[:500]
        print(f'GitHub Pages build request returned {exc.code}: {detail}')
        raise SystemExit(1) from exc
    except OSError as exc:
        print(f'GitHub Pages build request failed: {exc}')
        raise SystemExit(1) from exc
