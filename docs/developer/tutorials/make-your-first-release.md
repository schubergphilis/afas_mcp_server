...
6. **Updates the changelog** — `cz changelog` regenerates `docs/changelog.md` and commits it. Signing follows your `commit.gpgsign` setting; if signing is configured but no key can be reached — the usual case in CI — the commit is made unsigned and says so. Nothing is committed when the changelog is already current.
...
- [The scaffold](../index.md) — what the release shipped beyond the wheel (SBOM and security overview).
